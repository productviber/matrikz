...
interface SendWithProviderOptions {
  ...
  /** Canonical local message id generated by visibility-marketing. */
  messageId?: string;
...
export async function sendWithProvider(
  ...
  if (provider === 'sendgrid') {
    return sendViaSendGrid(env, to, subject, html, options);
  }
...
async function sendViaBrevo(
  ...
  if (options?.messageId) tags.push(`msg:${options.messageId}`);
  if (options?.templateKey) tags.push(`tpl:${options.templateKey}`);
  if (tags.length > 0) payload.tags = tags;

  const headers: Record<string, string> = {};
  if (options?.messageId) {
    headers['X-VM-Message-Id'] = options.messageId;
  }
  if (options?.sendId != null) {
    headers['X-VM-Send-Id'] = String(options.sendId);
  }
...
async function sendViaSendGrid(
  ...
  if (options?.messageId) customArgs.vm_message_id = options.messageId;
  if (options?.sendId != null) customArgs.vm_send_id = String(options.sendId);
  if (options?.templateKey) customArgs.vm_template_key = options.templateKey;
  ...
  const res = await fetch(EMAIL_CONFIG.SENDGRID_API_URL, {
    ...
    body: JSON.stringify({
      ...
      headers,
      custom_args: Object.keys(customArgs).length > 0 ? customArgs : undefined,
    }),
  });
